        ...
        <main data-aos="fade-in" className='md:px-4 px-3 md:py-0 py-3'>
            ...
            >
                ...
                <div className='flex flex-col lg:flex-row gap-4 my-5 items-center justify-between' data-aos="fade-up">
                    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 w-full lg:w-[80%] items-center'>
                        ...
                        <div className="relative md:col-span-2">
                            <input
                                ...
                                className='w-full rounded-xl px-5 py-2 border bg-[#f9fafb] focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all duration-200 outline-none text-sm'
                                placeholder='Ҷустуҷӯ...'
                                type="search"
                            />
                        </div>
                    ...
                    <div className="hidden md:flex bg-gray-100 p-1 rounded-xl gap-1 shrink-0">
                        <button
                            onClick={() => setViewType('table')}
                            className={`p-2 rounded-lg transition-all ${viewType === 'table' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                            title="Намуди ҷадвал"
                        >
                            <TableIcon size={20} />
                        </button>
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${viewType === 'grid' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                ...
                <div className="space-y-4">
                    ...
                    {filteredBooks.length === 0 && !isLoading ? (
                        ...
                        <>
                            {viewType === 'table' ? (
                                <div className="overflow-x-auto overflow-y-clip rounded-xl border border-gray-200 bg-white shadow-sm md:max-w-full max-w-90">
                                    <table className="w-full text-left border-collapse min-w-[800px]">
                                        <thead>
                                            <tr className="bg-gray-50 border-b border-gray-200">
                                                <th className="p-4 text-xs font-bold uppercase text-gray-500">Номи китоб / Инвентар</th>
                                                ...
                                                <th className="p-4 text-xs font-bold uppercase text-gray-500">Соли нашр</th>
                                                <th className="p-4 text-xs font-bold uppercase text-gray-500">Ҳолат</th>
                                                <th className="p-4 text-xs font-bold uppercase text-gray-500">Статус</th>
                                            </tr>
                                        </thead>
                                                    ...
                                                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                                        <td className="p-4">
                                                            <div className="flex flex-col">
                                                                <span className="font-semibold text-gray-800">{item.textbook.title}</span>
                                                                <span className="text-[10px] text-blue-500 font-mono bg-blue-50 w-fit px-1 rounded">{item.inventory_number}</span>
                                                            </div>
                                                        </td>
                                                        <td className="p-4 text-gray-600 font-mono text-sm">#{item.id}</td>
                                                        <td className="p-4 text-sm text-gray-700">{item.textbook.grade}</td>
                                                        <td className="p-4 text-sm text-gray-700">{item.textbook.publication_year}</td>
                                                        <td className="p-4">
                ...
                <div className="flex md:flex-row flex-col-reverse justify-between items-center mt-5 p-4 bg-gray-50 rounded-xl border">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="px-4 py-2 border rounded-lg bg-white hover:bg-gray-100 disabled:opacity-50 transition-all"
                        >
                            Қаблӣ
                        </button>
