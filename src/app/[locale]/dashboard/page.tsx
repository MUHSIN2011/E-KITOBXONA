...
function Page() {
    ...
    if (isError) return <div className="flex justify-center flex-col items-center h-[80vh] gap-3">
        ...
        <div
            ...
            className='flex gap-2 items-center border bg-blue-600 text-white rounded-sm px-3 py-1 cursor-pointer hover:bg-blue-700 transition-colors'
        >
            <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-white"></div>
            <button className='cursor-pointer'>{t('refreshButton')}</button>
        </div>
    </div>
            ...
            <div className="px-3 py-3">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-blue-100 dark:border-0 shadow-sm gap-4" data-aos="fade-down">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-300">{t('bookFundControl')}</p>
                    </div>
                    <div className="flex gap-3">
                        ...
                        >
                            <Lock className="w-4 h-4" /> {isClosing ? t('closing') : t('closeYear')}
                        </Button>
                        <Dialog>
                            ...
                            <DialogContent className='dark:bg-gray-900'>
                                ...
                                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                                    <div>
                                        <Label className='my-2'>{t('yearName')}</Label>
                                        <Input {...register("name", { required: true })} placeholder={t('yearPlaceholder')} />
                                    </div>

                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <Label className='my-2'>{t('startDate')}</Label>
                                            <Input type="date" {...register("start_date", { required: true })} />
                                        </div>
                                        <div>
                                            <Label className='my-2'>{t('endDate')}</Label>
                ...
                <section data-aos="fade-up">
                    <div className="grid grid-cols-1 mb-8 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div data-aos="fade-right" data-aos-delay="200">
                            <Card
                                NameRole={t('totalBooks')}
                                cnt={books?.total.toString() || '0'}
                                Icons={<BookOpen />}
                                description={t('inMinistry')}
                            />
                        </div>
                        <div data-aos="fade-right" data-aos-delay="300">
                            <Card
                                path='/ministry'
                                NameRole={t('totalSchools')}
