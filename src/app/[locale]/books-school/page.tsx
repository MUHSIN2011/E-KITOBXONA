...
function Page() {
    ...
    const totalBooks = summaryData?.items?.reduce((acc: number, item: any) => acc + (item.total_copies || 0), 0) || 0;

    const newBooksCount = summaryData?.items?.reduce((acc: number, item: any) => acc + (item.new_count || 0), 0) || 0;

    const rentedBooksCount = summaryData?.items?.reduce((acc: number, item: any) => acc + (item.rented_copies || 0), 0) || 0;

    const damagedBooksCount = summaryData?.items?.reduce((acc: number, item: any) => acc + (item.old_count || 0), 0) || 0;
